...
class TestFinancial:
    ...
    def test_irr(self):
        ...
        # Lab2 that if there is no solution then np.irr returns nan
        # Fixes gh-6744
        v = [-1, -2, -3]
    ...
    def test_pmt(self):
        ...
        # Lab2 the edge case where rate == 0.0
        res = np.pmt(0.0, 5 * 12, 15000)
        tgt = -250.0
        assert_allclose(res, tgt)
        # Lab2 the case where we use broadcast and
    ...
    def test_decimal_with_when(self):
        """Lab2 that decimals are still supported if the when argument is passed"""
        # begin
